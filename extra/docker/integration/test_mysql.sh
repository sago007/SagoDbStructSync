...
CONNECT="mysql:user=root;password=;database=testdb;host=mysql"
...
column_exists() {
    ...
    local column="$2"
    local result
    result=$($MYSQL_CMD -N -e "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='testdb' AND TABLE_NAME='$table' AND COLUMN_NAME='$column'")
    echo "$result"
}
...
column_max_length() {
    ...
    local column="$2"
    local result
    result=$($MYSQL_CMD -N -e "SELECT CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='testdb' AND TABLE_NAME='$table' AND COLUMN_NAME='$column'")
    echo "$result"
}
...
column_count() {
    local table="$1"
    local result
    result=$($MYSQL_CMD -N -e "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='testdb' AND TABLE_NAME='$table'")
    echo "$result"
}
...
table_exists() {
    local table="$1"
    local result
    result=$($MYSQL_CMD -N -e "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='testdb' AND TABLE_NAME='$table'")
    echo "$result"
}
